<template>
  ...
  <div class="p-2 rounded-lg mb-2">
    <p class="text-xl font-bold">Gastos de Insumos</p>
  </div>
  <div>
    <DataTable :value="Insumos" tableStyle="min-width: 50rem" show-gridlines size="small" paginator :rows="5">
      <template #header>
      ...
      <Column header="Movimientos">
        <template #body="slotProps">
          <div class="flex items-center justify-center gap-2">
            <Button 
              label="Agregar" variant="text" 
              @click="idInsumo = slotProps.data.id, NuevoMovimiento = true" />
            <Button 
              label="Listar" variant="text" 
              @click="idInsumo = slotProps.data.id, MovimientosInsumo = true"/>
          </div>
        </template>
      </Column>
    </DataTable>
  ...
  <modalAgregarInsumo :open="AgregarInsumo" v-if="AgregarInsumo" @close="AgregarInsumo = false" @update="obtenerInsumos"
  ...
  <modalModificarInsumo :open="ModificarInsumo" :id="idInsumo" v-if="ModificarInsumo" @close="ModificarInsumo = false"
    @update="obtenerInsumos" @success="toast.add({ severity: 'success', summary: 'Modificado Exitoso', life: 3000 })"
...
const AgregarInsumo = ref(false) 
const ModificarInsumo = ref(false)

const MovimientosInsumo = ref(false)
